...
// [0, 10, 20, 30, 40, 50, 60, 61, 62, 63, 64, 65, 75, 85, 84, 83, 93, 94, 95, 96, 86, 76, 66, 56, 46, 36]
	...
	public FPTBasePoints(int[] stations) {
...
//		{
//			int[] stations = {0, 65, 93, 36};
//			System.out.println(Arrays.toString(new FPTBasePoints(stations).solve().toArray()));
//		}
//		{
...
//		}
	...
	private Node findNextBasePoint(Node from, Node to, Node over) {
		int nodeX, nodeY;
	...
	private List<Integer> getMinPathLength() {
		...
		int currStationInd = 0;
		int currentPoint = stations[currStationInd];
		int nextPoint = stations[currStationInd + 1];
		for (int st : stations) {
//			if (st) {
//				Node basePoint = findNextBasePoint(new Node(currentPoint), new Node(nextPoint), )
//			}
		}
//
//		while (true) {
